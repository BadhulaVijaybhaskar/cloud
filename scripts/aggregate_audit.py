...
class AuditAggregator:
    ...
    def generate_markdown_report(self, matrix: Dict[str, Any]) -> str:
        ...
        for policy in matrix['policies']:
            ...
            implemented = "✅" if policy['implemented'] else "❌"
            enforced = "✅" if policy['enforced'] else "❌"
            
            md.append(f"| {policy['name']} | {policy['category']} | {implemented} | {enforced} | {status_emoji} {policy['status']} | {policy['notes']} |")
    ...
    def generate_csv_report(self, matrix: Dict[str, Any]) -> str:
