...
if [ "$SIMULATION_MODE" = "false" ]; then
  ...
  curl -sSf "${EVENT_BUS_URL:-redis://localhost:6379}" || {
    echo "ERROR: Event bus (Redis) not reachable" >> reports/$COMPONENT_NAME/precheck.log
    exit 2
  }
  
  # Check LangGraph service for integration
  curl -sSf "${LANGGRAPH_URL:-http://localhost:8080}/health" || {
    echo "WARNING: LangGraph service not reachable - integration may be limited" >> reports/$COMPONENT_NAME/precheck.log
  }
...
required_vars=("COMPONENT_NAME" "SERVICE_PORT" "REGISTRY_DB_URL" "EVENT_BUS_URL")
